...
describe('DockerController', () => {
    ...
    describe('pullImage', () => {
        it('should pull an image successfully', async () => {
            ...
            mockDocker.pull.mockImplementation((image: string, callback: (err: Error | null, stream: EventEmitter) => void) => {
                callback(null, mockStream);
            });

            mockDocker.modem.followProgress.mockImplementation((stream: EventEmitter, onFinished: (err: Error | null, output: any[]) => void) => {
                onFinished(null, []);
            });

            await expect(dockerController.pullImage('test-image')).resolves.not.toThrow();
            expect(mockDocker.pull).toHaveBeenCalledWith('test-image', expect.any(Function));
        });

        it('should handle errors when pulling an image', async () => {
            mockDocker.pull.mockImplementation((image: string, callback: (err: Error) => void) => {
                callback(new Error('Pull failed'));
            });

            await expect(dockerController.pullImage('test-image')).rejects.toThrow('Pull failed');
        });
    });
    ...
    describe('createNetwork', () => {
        it('should create a network if it does not exist', async () => {
            ...
            expect(mockDocker.createNetwork).toHaveBeenCalledWith({ Name: 'test-network' });
        });
