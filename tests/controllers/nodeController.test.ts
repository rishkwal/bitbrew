...
describe('NodeController', () => {
    ...
    beforeEach(() => {
        ...
        nodeController = new NodeController(dockerControllerMock, stateControllerMock);
    });

    describe('createNode', () => {
        it('should create a new node successfully', async () => {
            const node: NodeConfig = { name: 'test-node', port: 18444, rpcPort: 18443, status: 'initialized', dataDir: '/fake/path' };
            vi.spyOn(fs, 'existsSync').mockReturnValue(false);
            vi.spyOn(fs, 'mkdirSync').mockImplementation(() => undefined);
            dockerControllerMock.docker.listImages.mockResolvedValue([{ Id: 'image-id' }]);
            mockContainer.inspect.mockRejectedValue(new Error('Container not found'));

            await nodeController.createNode(node);

            expect(fs.mkdirSync).toHaveBeenCalledWith(node.dataDir, { recursive: true });
            expect(dockerControllerMock.createContainer).toHaveBeenCalledWith(expect.objectContaining({
                Image: 'bitcoin-regtest:1.0',
                name: node.name,
            }));
            expect(stateControllerMock.setNodeStatus).toHaveBeenCalledWith(node.name, 'stopped');
        });

        it('should not create a node if it already exists', async () => {
            const node: NodeConfig = { name: 'existing-node', port: 18444, rpcPort: 18443, status: 'initialized', dataDir: '/fake/path' };
            mockContainer.inspect.mockResolvedValue({});
            dockerControllerMock.docker.listImages.mockResolvedValue([{ Id: 'bitcoin-regtest' }]);
        ...
        it('should pull the image if it does not exist', async () => {
            ...
            expect(dockerControllerMock.docker.pull).toHaveBeenCalledWith('bitcoin-regtest:1.0');
    ...
    describe('startNode', () => {
        it('should start a node successfully', async () => {
            const node: NodeConfig = { name: 'test-node', port: 18444, rpcPort: 18443, status: 'stopped', dataDir: '/fake/path' };
            mockContainer.inspect.mockResolvedValue({ State: { Running: false } });

            await nodeController.startNode(node);

            expect(mockContainer.start).toHaveBeenCalled();
            expect(stateControllerMock.setNodeStatus).toHaveBeenCalledWith(node.name, 'running');
        });

        it('should not start a node that is already running', async () => {
            const node: NodeConfig = { name: 'running-node', port: 18444, rpcPort: 18443, status: 'running', dataDir: '/fake/path' };
        ...
        it('should handle errors when starting a node', async () => {
            const node: NodeConfig = { name: 'error-node', port: 18444, rpcPort: 18443, status: 'stopped', dataDir: '/fake/path' };
            mockContainer.inspect.mockResolvedValue({ State: { Running: false } });
    ...
    describe('stopNode', () => {
        ...
        it('should not stop a node that is already stopped', async () => {
            const node: NodeConfig = { name: 'stopped-node', port: 18444, rpcPort: 18443, status: 'stopped', dataDir: '/fake/path' };
            mockContainer.inspect.mockResolvedValue({ State: { Running: false } });
    ...
    describe('waitForNodeReady', () => {
        it('should resolve when node is ready', async () => {
            const node: NodeConfig = { name: 'test-node', port: 18444, rpcPort: 18443, status: 'running', dataDir: '/fake/path' };
            const mockExec = {
            ...
            await expect(nodeController.waitForNodeReady(node, 1, 100)).resolves.not.toThrow();
        });
        ...
        it('should throw an error if node is not ready after max retries', async () => {
            ...
            await expect(nodeController.waitForNodeReady(node, 1, 100)).rejects.toThrow('Timeout waiting for node test-node to be ready');
        });
    });

    describe('connectNode', () => {
        it('should connect nodes successfully', async () => {
            const sourceNode: NodeConfig = { name: 'source-node', port: 18444, rpcPort: 18443, status: 'running', dataDir: '/fake/path' };
            const targetNode: NodeConfig = { name: 'target-node', port: 18445, rpcPort: 18444, status: 'running', dataDir: '/fake/path' };
            const mockExec = {
                start: vi.fn().mockResolvedValue({}),
            };
            mockContainer.exec.mockResolvedValue(mockExec);
