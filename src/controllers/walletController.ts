...
class WalletController {
  ...
  async listWallets() {
    ...
    console.table(
      this.wallets.map((wallet: Wallet) => {
        return {
          name: wallet.name,
          node: wallet.node,
        };
  ...
  async getBalance(walletName: string) {
    ...
    try {
      ...
      this.docker.execCommand(
        wallet.node,
        `bitcoin-cli -rpcwallet=${walletName} getbalances`,
      );
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(error.message);
  ...
  async transferBitcoin(fromWallet: string, toWallet: string, amount: number) {
    ...
    try {
      this.docker.execCommand(
        from.node,
        `bitcoin-cli -rpcwallet=${fromWallet} sendtoaddress ${toAddress} ${amount}`,
      );
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(error.message);
  ...
  private async loadWallet(walletName: string) {
    ...
    try {
      await this.docker.getExecOutput(
        wallet.node,
        `bitcoin-cli -rpcwallet=${walletName} loadwallet ${walletName}`,
      );
    } catch (error: unknown) {
      if (error instanceof Error) {
  ...
  private async getAddress(walletName: string): Promise<string | undefined> {
    ...
    await this.loadWallet(walletName);
    const address = await this.docker.getExecOutput(
      wallet.node,
  ...
  public async mineBlocks(walletName: string, blocks: number) {
    clilog.info(
      `Mining ${blocks} block(s) and sending reward to ${walletName}`,
    );
    const wallet = this.wallets.find((wallet) => wallet.name === walletName);
    if (wallet === undefined) {
      throw new Error(`Wallet ${walletName} not found`);
