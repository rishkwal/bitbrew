...
describe('NetworkController', () => {
  ...
  describe('startNetwork', () => {
    it('should start the network successfully', async () => {
      ...
      expect(dockerControllerMock.createNetwork).toHaveBeenCalledWith(
        'bitbrew',
      );
      nodes.forEach((node) => {
    ...
    it('should handle errors when starting the network', async () => {
      dockerControllerMock.createNetwork.mockRejectedValue(
        new Error('Network creation failed'),
      );

      await expect(networkController.startNetwork()).rejects.toThrow(
  ...
  describe('addNode', () => {
    it('should add a new node successfully', async () => {
      ...
      expect(nodeControllerMock.createNode).toHaveBeenCalledWith(
        expect.objectContaining({ name: nodeName }),
      );
      expect(networkController.nodes).toContainEqual(
        expect.objectContaining({ name: nodeName }),
      );
      expect(stateControllerMock.saveState).toHaveBeenCalledWith(
        networkController.nodes,
  ...
  describe('connectNodes', () => {
    it('should connect nodes successfully', async () => {
      ...
      expect(nodeControllerMock.connectNode).toHaveBeenCalledWith(
        sourceNode,
        targetNode,
      );
    });
    ...
    it('should throw an error when connecting non-existent nodes', async () => {
      ...
      await expect(
        networkController.connectNodes('non-existent', ['also-non-existent']),
  ...
  describe('cleanNetwork', () => {
    it('should clean the network successfully', async () => {
      ...
      nodes.forEach((node) => {
        expect(
          dockerControllerMock.getContainer(node.name).remove,
        ).toHaveBeenCalledWith({ force: true });
      });
      expect(dockerControllerMock.removeNetwork).toHaveBeenCalledWith(
        'bitbrew',
      );
    ...
    it('should handle errors when cleaning the network', async () => {
      ...
      dockerControllerMock
        .getContainer(errorNode.name)
        .remove.mockRejectedValue(new Error('Container removal failed'));
      console.log = vi.fn(); // Mock console.log to check error logging

      await networkController.cleanNetwork();

      expect(console.log).toHaveBeenCalledWith(
        'Error removing container for error-node',
      );
      expect(dockerControllerMock.removeNetwork).toHaveBeenCalledWith(
        'bitbrew',
      );
